...
if (isMobile()){ // On mobile, the link has to be tapped two times to trigger redirection. First tap shows the tooltip
    for (i=0;i<links.length;i++){
        links[i].addEventListener("click",(evt) => { // If the tooltip is equal to the link the user clicked, that means it is the last link pressed by user (double tap simulation)
            if (tooltip.text() === evt.target.getAttribute('data-tooltip')){
                alert(tooltip.text(),evt.target.getAttribute('data-tooltip'))
...
window.addEventListener('mousemove',(evt) => {
    ...
    if ((evt.clientY > height-quarter) && ((width-fifth*3 < evt.clientX) && (evt.clientX < width-fifth*2)) || evt.target.classList.contains("show-menu")){ // Checking is user is on the bottom (middle fifth of the screen width AND last quarter of the screen's height) of the screen OR still in the menu (edge case for small screens where part of the menu would be outside the first condition's range)
        if (!animationPlaying && !iconbar.hasClass('show-menu')){ // Trying to show menu, making sure it is not already shown and an animation is not currently palying
            ...
            floating.addClass('show-menu').removeClass('hide-menu')
            tooltip.addClass('show-menu').removeClass('hide-menu')
            notches.addClass('show-menu').removeClass('hide-menu').addClass('moving')
        }
    } else {
        if (!animationPlaying && !iconbar.hasClass('hide-menu')){ // Trying to show menu, making sure it is not already shown and an animation is not currently palying
            animationPlaying = true
            iconbar.addClass('hide-menu').removeClass('show-menu')
            floating.addClass('hide-menu').removeClass('show-menu')
            tooltip.addClass('hide-menu').removeClass('show-menu')
            notches.addClass('hide-menu').removeClass('show-menu')
        }
    }
})

iconbar.on('animationend',() => {
    animationPlaying = false
    notches.removeClass('moving') // To avoid weird bug that makes .notch transitions not working after the animation
})
